...
*/
    ...
    public: // <<---------------------------------------//public
        ...
        uint8_t bytesRead = 0;
        int8_t status = 0;
        ssize_t bytesWrittenbyWrite = 0;
        ssize_t bytesReadbyRead = 0;
        ...
        */
        uint8_t sendData(const uint16_t &messageLen, const uint8_t packetID = 0)
        {
            ...
            bytesWrittenbyWrite = 0; // Venky edits
            numBytesIncl = packet.constructPacket(messageLen, packetID);
            bytesWrittenbyWrite = write(serial_port, packet.preamble, sizeof(packet.preamble));
            bytesWrittenbyWrite += write(serial_port, packet.txBuff, numBytesIncl);
            bytesWrittenbyWrite += write(serial_port, packet.postamble, sizeof(packet.postamble));
        ...
        */
        uint8_t available()
        {
            ...
            if (fion) // Venky edits: fion is the number of bytes available to read.
            {
                ...
                {
                    bytesReadbyRead = read(serial_port, &recChar, sizeof(recChar));
