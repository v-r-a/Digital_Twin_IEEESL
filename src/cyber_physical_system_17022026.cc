...
void controller_SS_OC(const mjModel *m, mjData *d, double des_torso_quat[4], double des_torso_w[3], double des_qj[12], double des_qjdot[12]);
                          ...
                          RobotDataToRead *read_in)
{
    ...
    else
    {
        ...
        while (dExp->time < (tstart + 0.02))
        {
            ...
            double shadow_torso_quat[4], shadow_torso_Rmat[9];
            mju_copy(shadow_torso_quat, dExp->qpos + 3, 4);
            mju_quat2Mat(shadow_torso_Rmat, shadow_torso_quat);
            ...
            double err_Rmat[9];
            mju_mulMatTMat(err_Rmat, imu_corrected_torso_Rmat, shadow_torso_Rmat, 3, 3, 3);
            ...
            //***************************************************************************************************************************
            // SS + OC controller
            controller_SS_OC(mm, dExp, imu_corrected_torso_quat, w_rel, extrp_q, zz);
...
void controller_SS_OC(const mjModel *m, mjData *d, double des_torso_quat[4], double des_torso_w[3], double des_qj[12], double des_qjdot[12])
{
    if (imu_fusion.load())
    {
        ...
        double Kp_ori_fb_acc = 12.0;
        double Kd_ori_fb_acc = 1.6;
        double err_quat[3], err_w[3];

        mju_subQuat(err_quat, des_torso_quat, d->qpos + 3); // error quat = des * inv(curr)
        mju_scl3(err_quat, err_quat, Kp_ori_fb_acc);        // Kp * err_quat

        mju_sub3(err_w, des_torso_w, d->qvel + 3); // error w = des - curr
        mju_scl3(err_w, err_w, Kd_ori_fb_acc);     // Kd * err_W

        mju_add3(cmd_fb_acc + 3, err_quat, err_w);  // cmd acc = Kp*err_quat + Kd*err_w
        mju_addTo3(cmd_fb_acc + 3, des_fb_acc + 3); // add desired acc
        ...
        mju_scl(Kp_hold_minimal, Kp_hold, (1. - fac), 12);
        mju_scl(Kd_hold_minimal, Kd_hold_2, (1. - fac), 12);
        for (int i = 0; i < 12; i++)
        {
